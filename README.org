...
If you're a NixOS user or just a Nix package manager enjoyer, you're in luck. Each project comes with a flake that handles everything involved in the ~build~ and ~run~ steps."
...
There’s also a fully set up development shell with all the necessary dependencies:
#+begin_src shell
